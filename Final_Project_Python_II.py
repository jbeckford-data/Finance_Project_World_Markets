...
# 7: Claim is that fractal and multi-fractals generate a more realistic picture of market
# risks than log-normal.  Considering the last 10 years of NASDAQ, write a program to check whether
# fractal geometry could have better predicted market movements than log-normal.  Explain with graphs.
...
symbols = ['^DJI', '^GSPC', '^IXIC', '^GDAXI', '^FTSE', '^HSI', '^KS11', '^NSEI']
...
def plot_ret(stock):
    mu = stock['Return'][1:].mean()
    sigma = stock['Return'][1:].std()
    ...
    ax1.plot(x, mlab.normpdf(x, mu, sigma), color='blue')
    ax2.hist(stock['Return'][1:], alpha=.5, bins=50, label=stock['minor'].iloc[1])
    plt.legend()
    plt.show()

for stock in syms:
    plot_ret(stock)
    tvalue = sci.ttest_1samp(stock['Return'][1:],0)
    #print(tvalue)
    if tvalue[1] < .05:
...
def plot_boxes(df):
    ...
    plt.ylabel('Return')
    plt.xlabel(stock['minor'].iloc[1])
    plt.boxplot(stock['Return'][1:],flierprops=green_diamond)
    plt.show()
    

# For each stock, run a Kurtosis test to determine if normally distributed and plot returns in boxplot.
for stock in syms:
    kurt = sci.kurtosistest(stock['Return'][1:])
    plot_boxes(stock)
